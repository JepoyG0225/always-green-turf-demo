...
// (js/supabase-client.js), so /admin shares its project, anon key, and login
// session. Sign in once and both /admin and /admin work. No secrets here.
...
export async function requireSession() {
  ...
  if (!data?.session) { window.location.replace("/admin/login"); return null; }
  return data.session;
}
